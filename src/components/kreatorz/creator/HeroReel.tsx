...
function EmbedPlayer({ embed, autoplay, aspectClass, maxHeightClass }: { embed: EmbedInfo; autoplay: boolean; aspectClass: string; maxHeightClass: string }) {
...
function NativePlayer({ reel, aspectClass, maxHeightClass, embedded }: { reel: HeroReelData; aspectClass: string; maxHeightClass: string; embedded?: boolean }) {
      ...
      {(reel.title || reel.cta_label) && (
        <div className="absolute bottom-0 inset-x-0 z-[4] p-4 pb-5">
          ...
          {reel.cta_label && reel.cta_url && (
            <a href={reel.cta_url} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()}
...
export default function HeroReel({ reel, embedded }: Props) {
  ...
  return (
    ...
    >
      {embed ? (
        <>
          <EmbedPlayer embed={embed} autoplay={reel.playback_mode === "autoplay"} aspectClass={aspectClass} maxHeightClass={maxHeightClass} />
          {(reel.title || reel.cta_label) && (
        ...
        <NativePlayer reel={reel} aspectClass={aspectClass} maxHeightClass={maxHeightClass} embedded={embedded} />
      )}
    </motion.div>
  );
}
